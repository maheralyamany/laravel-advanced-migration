...
use AdvancedMigration\Helpers\ValueToString;
use AdvancedMigration\Helpers\WritableTrait;

/**
 * Class ColumnDefinition
 * @package AdvancedMigration\Definitions
 */
class ColumnDefinition
{
    use WritableTrait;
    ...
    protected bool $useCurrent = true;
    ...
    protected function guessLaravelMethod()
    {
        ...
        if ($this->methodName === 'year') {
            return [$this->columnName, 'year', []];
        }
        ...
        if (config('advanced-migration-generator.definitions.prefer_unsigned_prefix') && $this->unsigned) {
    ...
    public function render(): string
    {
        ...
        $is_data = ($this->defaultValue !== null && ($this->defaultValue == "current_timestamp()" || str_contains($this->defaultValue, 'timestamp')));
        if ($this->defaultValue !== null && !$is_data) {
            $initialString .= '->default(';
            $initialString .= ValueToString::make($this->defaultValue, false);
            $initialString .= ')';
        }
        if ($this->useCurrent && $is_data) {
        ...
        if ($this->index) {
            $indexName = '';
            if (count($this->indexDefinitions) === 1 && config('advanced-migration-generator.definitions.use_defined_index_names')) {
        ...
        if ($this->primary && !$this->isPrimaryKeyMethod($finalMethodName)) {
            $indexName = '';
            if (count($this->indexDefinitions) === 1 && config('advanced-migration-generator.definitions.use_defined_primary_key_index_names')) {
        ...
        if ($this->unique) {
            $indexName = '';
            /*  if (count($this->indexDefinitions) === 1) {
            $indexName = ValueToString::make($this->indexDefinitions[0]->getIndexName());
            } */
        ...
        $comm = $this->getMComment();
        if ($comm !== null) {
            $initialString .= '->comment(' . ValueToString::make(str_replace('"', '\"', $comm), false, false) . ')';
        }
