...
class TableFormatter
{
    ...
    public function getStubPath(): string
    {
        ...
        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/' . $driver . '-table.stub'))) {
            return $overridden;
        }

        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/table.stub'))) {
            return $overridden;
        }
    ...
    public function getStubCreatePath(): string
    {
        ...
        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/' . $driver . '-table-create.stub'))) {
            return $overridden;
        }

        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/table-create.stub'))) {
            return $overridden;
        }
    ...
    public function getStubModifyPath(): string
    {
        ...
        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/' . $driver . '-table-modify.stub'))) {
            return $overridden;
        }

        if (file_exists($overridden = resource_path('stubs/vendor/advanced-migration-generator/table-modify.stub'))) {
            return $overridden;
        }
    ...
    public function stubNameVariables($index): array
    {
        ...
        return [
            ...
            'TableName' => $tableName,
            'Timestamp' => app('advanced-migration-generator:time')->format('Y_m_d_His'),
            'Index' => (string) $index,
            'IndexedEmptyTimestamp' => '0000_00_00_' . str_pad((string) $index, 6, '0', STR_PAD_LEFT),
            'IndexedTimestamp' => app('advanced-migration-generator:time')->clone()->addSeconds($index)->format('mdHis'),
            /* 'IndexedTimestamp' => app('advanced-migration-generator:time')->clone()->addSeconds($index)->format('Y_m_d_His'), */
        ];
    }
    ...
    public function getSchema($tab = ''): string
    {
        ...
        /*   $softDeletes = $this->tableDefinition->getSoftDeletes();
        if (!empty($softDeletes)) {
        $formatter->line($softDeletes);
        } */

        $comment = $this->tableDefinition->getTableComment();

        if (!empty($comment)) {
            $formatter->line($comment);
        }
